...
public class ThemeStyles {
    ...
    public static String render(Map<String, String> params, boolean cache, boolean inline, boolean virtualHosting) {
        ...
        if (themeDescriptor == null) {
            log.warn(String.format("Could not resolve theme descriptor for name '%s'", themeName));
        }
        ...
        if (inline) {
            ...
            return String.format("<style type=\"text/css\">\n%s\n</style>",
                    generateThemeStyles(themeName, themeDescriptor, basePath, collectionName, includeDate));
        }
        ...
        if (cache) {
            ...
            Long lastModifiedOnService = Manager.getThemeManager().getLastModified(themeName);
        ...
        return String.format(
                ...
                cssPath, themeName, themeName, path, basePath, collectionName, Long.valueOf(timestamp));
    }

    public static String generateThemeStyles(String themeName, ThemeDescriptor themeDescriptor, String basePath,
        ...
        for (Style style : themeManager.getSortedNamedStyles(themeName)) {
            sb.insert(
                    0,
                    CSSUtils.styleToCss(style, style.getSelectorViewNames(), IGNORE_VIEW_NAME, IGNORE_CLASSNAME, INDENT));
        ...
        for (Style style : themeManager.getStyles(themeName)) {
            sb.append(CSSUtils.styleToCss(style, style.getSelectorViewNames(), IGNORE_VIEW_NAME, IGNORE_CLASSNAME,
                    INDENT));
        }
        ...
        rendered = CSSUtils.expandVariables(rendered, basePath, collectionName, themeDescriptor);
        return rendered;
    }
}
